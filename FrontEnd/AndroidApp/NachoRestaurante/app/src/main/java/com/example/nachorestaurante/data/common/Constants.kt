package com.example.nachorestaurante.data.common

class Constants {
    companion object {
        const val BASE_URL = "http://informatica.wompwomp.com:2326/IgnacioLlorente_RestaurantRest-OscarApruebame/api/"
        const val CUSTOMERS = "customers"
        const val CUSTOMERSID = "customers/{id}"
        const val ORDERS = "orders"
        const val ORDERSID = "orders/{id}"
        const val ID = "id"
        const val NAME = "name"
        const val SURNAME = "surname"
        const val EMAIL = "email"
        const val PHONE = "phone"
        const val BIRTHDATE = "birthdate"
        const val CUSTOMERID = "customerId"
        const val ORDERDATE = "orderDate"
        const val TABLEID = "tableId"
        const val DELETEOK = "Deleted successfully"
        const val UNKNOWNERROR = "Unknown error"
        const val CUSTOMERNOTFOUND = "No customer found"
        const val ORDERNOTRETURNED = "No order returned"
    }
}